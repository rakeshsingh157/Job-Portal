    ...
    fetch(`PHP/job-details.php?job_id=${jobId}`)
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                const cuserId = job.cuser_id; // Get the cuser_id from the job data

                document.querySelector('h1').textContent = job.job_title;
                document.querySelector('.text-lg.text-gray-600').textContent = company.company_name;
                ...
                document.querySelector('.hidden.md\\:flex.flex-col.items-center.flex-shrink-0.pt-4 .text-sm').textContent = company.company_name;
                ...
                // Use a more robust way to find and update the Experience and Employment Type fields
                ...
                // Add redirection logic for company name and photo
                ...
                const companyPhotoLinks = document.querySelectorAll('.pink-box');
                companyPhotoLinks.forEach(photo => {
                    photo.style.cursor = 'pointer'; // Add a pointer cursor to indicate it's clickable
