<?php
// Start the PHP session to store OTPs and temporary user data
session_start();
...
// Fast2SMS API Key (VERY INSECURE TO HARDCODE IN PRODUCTION!)
// Set to an empty string for Canvas runtime injection.
// In a real production environment, this should be a secure environment variable (e.g., loaded from $_ENV).
...
// Function to send SMS OTP via Fast2SMS API
function sendSmsOtp($phoneNumber, $otp) {
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 30);          // Set a timeout for the cURL operation (30 seconds)
    // WARNING: In production, these SSL options should be set to 2 (VERIFYHOST) and 1 (VERIFYPEER) for security.
    // They are set to 0 here for easier local development/testing where SSL certificates might not be perfectly configured.
    curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0); 
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0); 
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET"); // Fast2SMS OTP API uses GET method for this endpoint

    // Execute cURL request
    $response = curl_exec($ch);
    $err = curl_error($ch); // Get any cURL errors
